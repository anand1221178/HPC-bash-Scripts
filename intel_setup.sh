...
[ -d /mnt/nfs_share/intel ] && echo "intel directory already exists" || mkdir -p /mnt/nfs_share/intel

cd /mnt/nfs_share/intel

#check for oneapi base toolkit and download it
if [ -f /mnt/nfs_share/intel/l_BaseKit_p_2023.1.0.46401.sh ]; then
    ...
    sh l_BaseKit_p_2023.1.0.46401.sh -a --silent --eula accept --install-dir /mnt/nfs_share/intel --action install --components intel.oneapi.lin.tbb.devel:intel.oneapi.lin.mkl.devel:intel.oneapi.lin.advisor:intel.oneapi.lin.dpl:intel.oneapi.lin.dpcpp-cpp-compiler:intel.oneapi.lin.vtune
    echo "oneapi basekit downloaded and installed successfully"
fi
#check for oneapi hpc toolkit and download it
if [ -f /mnt/nfs_share/intel/l_HPCKit_p_2023.1.0.46346.sh ]; then
    ...
    sh ./l_HPCKit_p_2023.1.0.46346.sh -a --silent --eula accept --install-dir /mnt/nfs_share/intel
    echo "oneapi hpckit downloaded and installed successfully"
fi

cd /mnt/nfs_share/intel
bash modulefiles-setup.sh
echo 'module use --append /mnt/nfs_share/intel/modulefiles' >> ~/.bashrc
source ~/.bashrc
